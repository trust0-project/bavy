...
pub enum MicroOp {
    ...
    Lb { rd: u8, rs1: u8, imm: i64, pc_offset: u16 },

    /// rd = zext(mem[rs1 + imm][7:0])
    Lbu { rd: u8, rs1: u8, imm: i64, pc_offset: u16 },

    /// rd = sext(mem[rs1 + imm][15:0])
    Lh { rd: u8, rs1: u8, imm: i64, pc_offset: u16 },

    /// rd = zext(mem[rs1 + imm][15:0])
    Lhu { rd: u8, rs1: u8, imm: i64, pc_offset: u16 },

    /// rd = sext(mem[rs1 + imm][31:0])
    Lw { rd: u8, rs1: u8, imm: i64, pc_offset: u16 },

    /// rd = zext(mem[rs1 + imm][31:0])
    Lwu { rd: u8, rs1: u8, imm: i64, pc_offset: u16 },

    /// rd = mem[rs1 + imm][63:0]
    Ld { rd: u8, rs1: u8, imm: i64, pc_offset: u16 },
    ...
    Sb { rs1: u8, rs2: u8, imm: i64, pc_offset: u16 },

    /// mem[rs1 + imm][15:0] = rs2[15:0]
    Sh { rs1: u8, rs2: u8, imm: i64, pc_offset: u16 },

    /// mem[rs1 + imm][31:0] = rs2[31:0]
    Sw { rs1: u8, rs2: u8, imm: i64, pc_offset: u16 },

    /// mem[rs1 + imm][63:0] = rs2
    Sd { rs1: u8, rs2: u8, imm: i64, pc_offset: u16 },
    ...
    Jal { rd: u8, imm: i64, pc_offset: u16, insn_len: u8 },

    /// Indirect jump: rd = pc + insn_len, pc = (rs1 + imm) & ~1
    Jalr { rd: u8, rs1: u8, imm: i64, pc_offset: u16, insn_len: u8 },

    /// Branch if rs1 == rs2
    Beq { rs1: u8, rs2: u8, imm: i64, pc_offset: u16, insn_len: u8 },

    /// Branch if rs1 != rs2
    Bne { rs1: u8, rs2: u8, imm: i64, pc_offset: u16, insn_len: u8 },

    /// Branch if rs1 < rs2 (signed)
    Blt { rs1: u8, rs2: u8, imm: i64, pc_offset: u16, insn_len: u8 },

    /// Branch if rs1 >= rs2 (signed)
    Bge { rs1: u8, rs2: u8, imm: i64, pc_offset: u16, insn_len: u8 },

    /// Branch if rs1 < rs2 (unsigned)
    Bltu { rs1: u8, rs2: u8, imm: i64, pc_offset: u16, insn_len: u8 },

    /// Branch if rs1 >= rs2 (unsigned)
    Bgeu { rs1: u8, rs2: u8, imm: i64, pc_offset: u16, insn_len: u8 },
    ...
    Csrrw { rd: u8, rs1: u8, csr: u16, pc_offset: u16 },

    /// CSR read-set: rd = csr, csr = csr | rs1
    Csrrs { rd: u8, rs1: u8, csr: u16, pc_offset: u16 },

    /// CSR read-clear: rd = csr, csr = csr & ~rs1
    Csrrc { rd: u8, rs1: u8, csr: u16, pc_offset: u16 },

    /// CSR read-write immediate
    Csrrwi { rd: u8, zimm: u8, csr: u16, pc_offset: u16 },

    /// CSR read-set immediate
    Csrrsi { rd: u8, zimm: u8, csr: u16, pc_offset: u16 },

    /// CSR read-clear immediate
    Csrrci { rd: u8, zimm: u8, csr: u16, pc_offset: u16 },
    ...
    ScW { rd: u8, rs1: u8, rs2: u8, pc_offset: u16 },

    /// Store-Conditional Doubleword
    ScD { rd: u8, rs1: u8, rs2: u8, pc_offset: u16 },

    /// Atomic swap (word/dword determined by `is_word`)
    AmoSwap { rd: u8, rs1: u8, rs2: u8, is_word: bool, pc_offset: u16 },

    /// Atomic add
    AmoAdd { rd: u8, rs1: u8, rs2: u8, is_word: bool, pc_offset: u16 },

    /// Atomic XOR
    AmoXor { rd: u8, rs1: u8, rs2: u8, is_word: bool, pc_offset: u16 },

    /// Atomic AND
    AmoAnd { rd: u8, rs1: u8, rs2: u8, is_word: bool, pc_offset: u16 },

    /// Atomic OR
    AmoOr { rd: u8, rs1: u8, rs2: u8, is_word: bool, pc_offset: u16 },

    /// Atomic MIN (signed)
    AmoMin { rd: u8, rs1: u8, rs2: u8, is_word: bool, pc_offset: u16 },

    /// Atomic MAX (signed)
    AmoMax { rd: u8, rs1: u8, rs2: u8, is_word: bool, pc_offset: u16 },

    /// Atomic MIN (unsigned)
    AmoMinu { rd: u8, rs1: u8, rs2: u8, is_word: bool, pc_offset: u16 },

    /// Atomic MAX (unsigned)
    AmoMaxu { rd: u8, rs1: u8, rs2: u8, is_word: bool, pc_offset: u16 },
}
...
mod tests {
    ...
    fn test_is_terminator() {
        assert!(MicroOp::Jal { rd: 0, imm: 0, pc_offset: 0, insn_len: 4 }.is_terminator());
        assert!(MicroOp::Beq { rs1: 0, rs2: 0, imm: 0, pc_offset: 0, insn_len: 4 }.is_terminator());
        assert!(MicroOp::Ecall { pc_offset: 0 }.is_terminator());
        assert!(!MicroOp::Addi { rd: 1, rs1: 0, imm: 0 }.is_terminator());
        assert!(!MicroOp::Add { rd: 1, rs1: 0, rs2: 0 }.is_terminator());
    }

    #[test]
    fn test_may_trap() {
        assert!(MicroOp::Ld { rd: 1, rs1: 0, imm: 0, pc_offset: 0 }.may_trap());
        assert!(MicroOp::Sd { rs1: 0, rs2: 0, imm: 0, pc_offset: 0 }.may_trap());
        assert!(MicroOp::Ecall { pc_offset: 0 }.may_trap());
        assert!(!MicroOp::Add { rd: 1, rs1: 0, rs2: 0 }.may_trap());
