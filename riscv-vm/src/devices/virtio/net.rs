    ...
    fn process_rx_queue(state: &mut VirtioNetState, dram: &Dram) -> Result<(), MemoryError> {
        ...
        loop {
            ...
            let packet = match state.backend.recv() {
                Ok(Some(pkt)) => {
                    log::debug!(
                        "[VirtioNet] Received {} byte packet from backend",
                        pkt.len()
                    );
                    pkt
                }
        ...
        if packets_delivered > 0 {
            ...
            if debug {
                log::debug!(
                    "[VirtioNet] RX: Delivered {} packets total",
                    packets_delivered
                );
            }
        }
        ...
        while last_avail_idx != avail_idx {
            ...
            if packet_data.len() > 12 {
                ...
                if let Err(e) = state.backend.send(actual_packet) {
                    ...
                    if debug {
                        log::debug!(
                            "[VirtioNet] TX: Sent {} byte packet (total: {})",
                            actual_packet.len(),
        ...
        let val = match offset {
            ...
            device::DEVICE_FEATURES_OFFSET => {
                if state.device_features_sel == 0 {
                    // Feature bits 0-31
                    (1u64 << device::VIRTIO_NET_F_MAC) | (1u64 << device::VIRTIO_NET_F_STATUS)
                } else {
                    0
                }
            }
                ...
                match aligned {
                    ...
                    4 => {
                        ...
                        (state.mac[4] as u64)
                            ...
                            | ((device::VIRTIO_NET_S_LINK_UP as u64) << 16)
                    }
                    8 => {
        ...
        match offset {
            ...
            device::QUEUE_PFN_OFFSET => {
                ...
                if pfn != 0 {
                    ...
                    queue.ready = true;
                    log::debug!(
                        "[VirtioNet] Queue {} configured: pfn={} desc=0x{:x} avail=0x{:x} used=0x{:x} num={}",
                        queue_sel,
                        pfn,
                        queue.desc,
                        queue.avail,
